  ...
  const getAllResDetails = () => {
    axios
      ...
      .then((res) => {
        ...
        // Combine objects by index
        const mergedDetails = allDetails.map((item, index) => {
          return { ...item, ...data[index] }; // Merge each object with the corresponding one from `data`
        });

        setResDetail(mergedDetails);
      })
      .catch((err) => {
        console.log(err);
      });
        ...
        <div className="container food-body px-10 py-10 md:mx-auto">
          <div className="flex flex-wrap md:-m-4 md:pl-6 foods-container">
            {filteredRes.map((food, index) => (
              ...
              >
                <a key={index} href={food.link} target="_blank" rel="noopener noreferrer">
                  <img src={food.url} alt={`Food item ${index + 1}`} />
                </a>
